...
#include "crypt_algorithms/crypt_gronsfeld.h"
#include "crypt_algorithms/crypt_vernam.h"
#include <QTextEdit>
#include <QLineEdit>
#include <QComboBox>
#include <QPushButton>
...
void MainWindow::setupUI()
{
    ...
    decryptButton->setMinimumHeight(35);
    generateRSAButton->setMinimumHeight(35);

    encryptButton->setMinimumWidth(120);
    decryptButton->setMinimumWidth(120);
    generateRSAButton->setMinimumWidth(180);

    removeSpacesCheckBox = new QCheckBox("Убрать пробелы", this);

    controlsLayout->addWidget(algorithmLabel);
    controlsLayout->addWidget(cipherSelect);
    controlsLayout->addWidget(keyEdit);
    controlsLayout->addWidget(removeSpacesCheckBox);
    ...
    inputTextPlain->setFont(QFont("Consolas", 14));
    ...
    inputTextBytes->setFont(QFont("Consolas", 14));
    ...
    outputTextPlain->setFont(QFont("Consolas", 14));
    ...
    outputTextBytes->setFont(QFont("Consolas", 14));
    ...
    publicKeyDisplay->setFont(QFont("Consolas", 10));
    ...
    privateKeyDisplay->setFont(QFont("Consolas", 10));
...
void MainWindow::setupAlgorithms()
{
    ...
    // алфавитные шифры
    cipherSelect->addItem("Caesar (Цезарь)");
    cipherSelect->addItem("Vigenere (Виженер)");
    cipherSelect->addItem("Gronsfeld (Гронсфельд)");
    cipherSelect->addItem("Vernam (Одноразовый блокнот)");
...
void MainWindow::updateBinaryDisplayVisibility()
{
    ...
    bool isAlphabet = isAlphabetAlgorithm(cipherSelect->currentText());
    removeSpacesCheckBox->setVisible(isAlphabet);
}
...
{
    QStringList alphabetAlgorithms = {"Caesar (Цезарь)", "Vigenere (Виженер)",
                                      "Gronsfeld (Гронсфельд)", "Vernam (Одноразовый блокнот)"};
...
{
    if (algorithm == "Caesar (Цезарь)")
    {
        return std::make_unique<Crypt_Caesar>();
    }
    else if (algorithm == "Vigenere (Виженер)")
    {
        return std::make_unique<Crypt_Vigenere>();
    }
    else if (algorithm == "Gronsfeld (Гронсфельд)")
    {
        return std::make_unique<Crypt_Gronsfeld>();
    }
    else if (algorithm == "Vernam (Одноразовый блокнот)")
    {
        return std::make_unique<Crypt_Vernam>();
    }
    return nullptr;
}
...
void MainWindow::Cipher_changed()
{
    ...
    else if (algorithm == "Caesar (Цезарь)")
    {
    ...
    {
        keyEdit->setPlaceholderText("Цифровой ключ (например: 1234)");
        keyEdit->setEnabled(true);
    }
    else if (algorithm == "Vernam (Одноразовый блокнот)")
    {
        keyEdit->setPlaceholderText("Одноразовый блокнот (должен быть не короче текста)");
        keyEdit->setEnabled(true);
    }
    else
    {
...
void MainWindow::Encrypt_clicked()
{
    ...
    else if (isAlphabetAlgorithm(algorithm))
    {
        ...
        // настройка удаления пробелов для алфавитных шифров
        if (algorithm == "Caesar (Цезарь)")
        {
            auto caesarCrypter = dynamic_cast<Crypt_Caesar*>(crypter.get());
            caesarCrypter->set_remove_spaces(removeSpacesCheckBox->isChecked());
        }
        else if (algorithm == "Vigenere (Виженер)")
        {
            auto vigenereCrypter = dynamic_cast<Crypt_Vigenere*>(crypter.get());
            vigenereCrypter->set_remove_spaces(removeSpacesCheckBox->isChecked());
        }
        else if (algorithm == "Gronsfeld (Гронсфельд)")
        {
            auto gronsfeldCrypter = dynamic_cast<Crypt_Gronsfeld*>(crypter.get());
            gronsfeldCrypter->set_remove_spaces(removeSpacesCheckBox->isChecked());
        }
        else if (algorithm == "Vernam (Одноразовый блокнот)")
        {
            auto vernamCrypter = dynamic_cast<Crypt_Vernam*>(crypter.get());
            vernamCrypter->set_remove_spaces(removeSpacesCheckBox->isChecked());
        }
